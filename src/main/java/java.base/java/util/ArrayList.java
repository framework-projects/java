        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     *
     * ArrayList的底层是由数组实现的，数组集合的默认大小为10
     */
     ...
     *
     * ArrayList的有参构造函数，传入的参数是数组集合的大小
     *
     * 如果预先可以知道或者能够估计出ArrayList中数组元素的个数，需要传入参数指定数组集合的大小
     *
     ...
     * ArrayList的无参构造函数
     *
     * 首件将默认大小的数组集合元素DEFAULTCAPACITY_EMPTY_ELEMENTDATA赋值给elementData。然后根据插入数据记录的个数和当前数组集合的大小size比较，
     ...
     *
     * 获取对象在数组集合ArrayList中的索引位置，如果数组集合ArrayList中没有这样的对象就返回 -1
     */
     ...
     *
     * 首先无论是指定索引位置index还是在数组集合的末尾执行add()操作，都会先确认ArrayList的当前数组集合的空间是否能够插入数据
     *
     * ArrayList默认每次自增50%的大小然后和满足需要的minCapacity比较，如果不能够满足数组大小的要求，就将数组的大小size扩充至满足
     * 需要的数组集合大小minCapacity
     *
     ...
     * indices).
     *
     * 首先进行边界检查，检查传入的索引位置index是否超过当前数组集合的大小，如果超过就抛出异常。
     * 如果index的值在数组集合大小范围内，就将index之后的数据整体向前移动一位，将最后一位的值清空。
     *
     ...
     *
     * 这里传入了一个对象，会在数组集合中进行遍历查找，寻找当前的数组集合ArrayList中是否存在这个对象
     * 如果数组集合ArrayList中存在这个对象就调用fastRemove()方法进行删除
     *
